<div class="header-spacer"></div>
<div class="content-wrapper">
	<section>
		<div class>
			<div class="row" style="margin: 75px 0 130px 0">
				<div class="col-lg-8" style="float: none; margin: 0 auto">
					<div class="crumina-module crumina-heading align-center">
						<h2 class="h1 heading-title">We'd Love to Hear from you</h2>
						<div class="heading-text">Want to find out how NABLE INVENT SOLUTION can solve problems?
							We're all geared up and waiting to talk with you!
						</div>
					</div>
				</div>
				<div class="col-lg-4 col-lg-offset-4">
					<img src="<?php echo base_url("assets/site/"); ?>img/home/contactnow.jpg" alt="map">
				</div>
			</div>
	</section>
	<!-- Contacts -->
	<section>
		<div class="container-fluid no-padding">

			<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12 no-padding">
				<section>
					<div class="container">
						<div class="row">
							<div class="col-lg-6 col-lg-offset-2 col-sm-12">
								<div class="dotted-map">
									<img src="<?php echo base_url("assets/site/"); ?>img/newmap.png" alt="map">

								</div>
							</div>
						</div>
					</div>
				</section>
			</div>

			<div class="col-lg-4 col-lg-offset-1 col-sm-12">


				<div class="crumina-module crumina-heading">
					<h3 class="heading-title">Get in Touch with Us</h3>

				</div>
				<div class="widget w-contacts w-contacts--style2 ">
					<!-- <div class="contact-item display-flex">
							<svg class="utouch-icon utouch-icon-placeholder-3">
								<use xlink:href="#utouch-icon-placeholder-3"></use>
							</svg>
							<span class="info">Sysorex Innovator Edge, 2nd Floor Karan Chambers, Vyapar Vihar Road, Bilaspur, (C.G.) India </span>
						</div> -->
					<div class="contact-item display-flex">
						<svg class="utouch-icon utouch-icon-telephone-keypad-with-ten-keys">
							<use xlink:href="#utouch-icon-telephone-keypad-with-ten-keys"></use>
						</svg>
						<div class="info-wrap">
							<span class="info">+91- 7880118100
								<!--<span>- HQ</span>-->
							</span>
						</div>
					</div>
					<div class="contact-item display-flex">
						<svg class="utouch-icon utouch-icon-message">
							<use xlink:href="#utouch-icon-message"></use>
						</svg>
						<a href="#" class="info">info@sysorexedge.com</a>
					</div>
					<div class="contact-item display-flex">
						<svg class="utouch-icon">
							<use xlink:href="#utouch-icon-world-map"></use>
						</svg>
						<div class="info-wrap">
							<span class="info">NABLE INVENT SOLUTION</span>
							a. North Carolina, <b>United States</b><br>
							b. Muscat, <b>Sultanate of Oman</b><br>
							c. Chhattisgarh, <b>India</b><br>
							<span class="info">SYSOREX IT SOLUTION</span>
							a. Abu Dhabi, <b>United Arab Emirates</b>
						</div>
						<!-- </div>

						<a href="#" class="btn btn--grey btn--with-shadow js-message-popup cd-nav-trigger">
							Send a Message
						</a> -->
					</div>

				</div>

			</div>
	</section>

	<!-- ... end Contacts -->

</div>


</html>